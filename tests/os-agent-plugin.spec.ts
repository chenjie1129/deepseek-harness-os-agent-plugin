...
describe('OS Agent Plugin Volcengine client', () => {
  it('mounts as a real Cordis plugin and registers all three tools', async () => {
    const ctx = new Context()
    await ctx.plugin(SystemPrompt)
    await ctx.plugin(ToolRuntime)
    const fiber = ctx.plugin({ name: 'os-agent-composition-test', inject, apply }, {})
    await fiber

    expect(ctx.tools.get('mobile_use_start_task')).toBeDefined()
    expect(ctx.tools.get('mobile_use_get_status')).toBeDefined()
    expect(ctx.tools.get('mobile_use_get_result')).toBeDefined()

    await fiber.dispose()
    expect(ctx.tools.get('mobile_use_start_task')).toBeUndefined()
  })

  it('signs a RunAgentTaskOneStep request and sends every configured field', async () => {
    ...
    expect(new Headers(init.headers).get('authorization')).toMatch(
      /^HMAC-SHA256 Credential=test-access\/20260821\/cn-north-1\/ipaas\/request,/,
    )
    expect(new Headers(init.headers).get('authorization')).toContain(
      'Signature=cf8bc2a432a777a4814d4976ccb98ff4f355c939eedfc230c4133493d5001a41',
    )
    expect(new Headers(init.headers).get('x-date')).toBe('20260821T000000Z')
    expect(JSON.parse(String(init.body))).toEqual({
      RunName: 'configured-name',
      PodId: 'pod-1',
      ProductId: 'product-1',
      UserPrompt: 'Open Settings',
      MaxStep: 12,
      Timeout: 90,
      ThreadId: 'thread-1',
      SystemPrompt: 'Be careful',
